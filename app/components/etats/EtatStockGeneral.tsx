...
interface EtatStockGeneralProps {
  structureId?: string;
  ministereId?: string;
}

export default function EtatStockGeneral({ structureId, ministereId }: EtatStockGeneralProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`/api/etats/stock/general?${params}`);
  ...
  return (
    <EtatImprimable titre="État Général du Stock">
      <EnteteDocument
        titre="ÉTAT GÉNÉRAL DU STOCK"
        sousTitre={`Au ${new Date().toLocaleDateString('fr-FR')}`}
